...
result = client.functions.enqueue(
    ...
    input={"doc_id": "01HX..."},
    trigger_id=f"grove:run:{run_id}",
    callback_url=f"https://grove.example.com/sinas/callbacks/{run_id}",
)
...
const { executionId } = await enqueueFunction(
  client,
  'grove/ingest_document',
  { doc_id: '01HX...' },
  {
    triggerId: `grove:run:${runId}`,
    callbackUrl: `https://grove.example.com/sinas/callbacks/${runId}`,
  },
);
```
...
Grove submits 50 documents for ingestion on behalf of a signed-in user.

1. Grove's backend, holding the user's bearer, calls `client.functions.enqueue(...)` once per document.
2. Each `execution_id` lands in a queue. A Sinas worker dequeues and runs `grove/ingest_document` under the user's identity.
3. The function calls Grove's `/api/v1/documents/{id}/mark-processed` with the per-execution access token. Grove's `SinasAuth` validates the token via Sinas's `/auth/me`, applies the user's permissions, persists the update.
4. When the function finishes, Sinas POSTs the result to `callback_url`. Grove updates the run's progress and notifies the user.

If `ingest_document` invokes a Sinas agent (`client.chats.invoke(...)`), the agent runs in-process under the same user; sub-agent fan-out is internal and doesn't re-cross the Grove boundary, so the function token's TTL never bites.
...

{
  ...
  "trigger_id_prefix": "grove:run:01HX",         // optional; per-child trigger_id becomes "{prefix}:{i}"
...
batch = client.functions.submit_batch(
    ...
    trigger_id_prefix=f"grove:run:{run.id}",
    batch_callback_url=f"https://grove.example.com/sinas/batch/{run.id}",
)
...
const batch = await submitFunctionBatch(
  ...
  {
    triggerIdPrefix: `grove:run:${runId}`,
    batchCallbackUrl: `https://grove.example.com/sinas/batch/${runId}`,
  },
);
